      ...
      <div style={{ background: 'var(--bg-surface)', border: '0.5px solid var(--border-color)', borderRadius: isMobile ? '10px' : '14px', width: '100%', maxWidth: isMobile ? '100%' : '860px', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', maxHeight: isMobile ? '65vh' : '520px', overflowY: isMobile ? 'auto' : 'visible' }}>
          ...
          <div style={{ flex: isMobile ? '1' : '0 0 300px', padding: '20px 18px', display: 'flex', flexDirection: 'column', overflowY: isMobile ? 'visible' : 'auto' }}>
            ...
            {/* Prop Firm label */}
            <label style={{ ...label, marginBottom: '8px' }}>Prop Firm</label>

            {/* Inline firm list */}
            <div style={{ background: 'var(--bg-page)', border: '0.5px solid var(--border-color)', borderRadius: '10px', overflow: 'hidden', marginBottom: '14px' }}>
                ...
                {firms.map(f => {
                  ...
                  return (
                    <div
                      ...
                      style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 13px', cursor: 'pointer', background: isMatch ? 'var(--green-bg-2)' : sel ? 'var(--green-bg)' : 'transparent', color: sel ? 'var(--brand)' : 'var(--text-soft)', fontFamily: ff, fontSize: '13px', borderBottom: '0.5px solid var(--border-color)' }}
                    ...
                    >
                      <span>🏛 {f}</span>
                      {sel && <span style={{ color: 'var(--brand)', fontSize: '13px' }}>✓</span>}
                    </div>
                  )
                })}
                <div
                  style={{ display: 'flex', alignItems: 'center', padding: '9px 13px', cursor: 'pointer', color: selectedFirm === 'Other' ? 'var(--brand)' : 'var(--text-faint-2)', background: selectedFirm === 'Other' ? 'var(--green-bg)' : 'transparent', fontFamily: ff, fontSize: '13px' }}
            ...
            <div style={{ marginBottom: '14px' }}>
              ...
              <div style={{ position: 'relative' }}>
                <div
                  style={{ width: '100%', background: 'var(--bg-page)', border: `0.5px solid ${selectedPhase ? 'var(--brand)' : 'var(--border-color)'}`, borderRadius: '8px', padding: '9px 12px', color: selectedPhase ? 'var(--text-primary)' : 'var(--text-faint-2)', fontFamily: ff, fontSize: '13px', cursor: (!isOther && !selectedType) ? 'not-allowed' : 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxSizing: 'border-box', opacity: (!isOther && !selectedType) ? 0.35 : 1 }}
                  onClick={() => { if (!isOther && !selectedType) return; setPhaseOpen(o => !o); setFirmOpen(false) }}
                >
