...
function getMonthlyData(trades, monthsCount = 12) {
  const map = {}
  trades.filter(t => t.pnl != null).forEach(t => {
...
export default function DailyBarChart({ trades = [], mobile = false }) {
  ...
  const noDataLabel = view === '7d' ? '7 days' : view === '30d' ? '30 days' : '12 months'
    ...
    return (
      <div style={{ padding: '10px 14px 10px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
          <span style={{ fontSize: '11px', color: 'var(--text-faint)', fontFamily: 'DM Sans, sans-serif' }}>{view === '12m' ? 'Monthly PnL' : 'Daily PnL'}</span>
          <div style={{ display: 'flex', gap: '4px' }}>
            ...
            <button style={toggleStyle(view === '12m')} onClick={() => setView('12m')}>12m</button>
          </div>
        </div>
        ...
        {!hasData ? (
          ...
          <ResponsiveContainer width="100%" height={52} style={{ WebkitTapHighlightColor: "transparent", outline: "none" }}>
            <BarChart data={data} barSize={view === '30d' ? 6 : view === '12m' ? 12 : 16} margin={{ top: 2, right: 0, bottom: 0, left: 0 }}>
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <h2 style={{ color: 'var(--text-primary)', fontFamily: 'Syne, sans-serif', fontSize: '15px', fontWeight: '600', margin: 0 }}>
          {view === '12m' ? 'Monthly P&L — Last 12 Months' : `Daily P&L — ${view === '7d' ? 'Last 7 Days' : 'Last 30 Days'}`}
        </h2>
        <div style={{ display: 'flex', gap: '6px' }}>
          ...
          <button style={toggleStyle(view === '12m')} onClick={() => setView('12m')}>Last 12 months</button>
        </div>
      </div>
      {!hasData ? (
        ...
        <ResponsiveContainer width="100%" height={180} style={{ WebkitTapHighlightColor: "transparent", outline: "none" }}>
          <BarChart data={data} barSize={view === '30d' ? 14 : view === '12m' ? 26 : 36}>
